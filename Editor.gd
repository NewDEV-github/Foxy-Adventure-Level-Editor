...
func build_level():
	...
	pck.pck_start(MainSingleton.level_path + "/" + MainSingleton.level_name + ".pck")
	...
	while not mode == "finished":
		dir.open("user://level_data/" + MainSingleton.level_name + "/%s/" % mode)
		dir.list_dir_begin()
		var file_name = dir.get_next()
		while not file_name == "":
			var av = ['.', '..']
			if not dir.current_is_dir():
				pck.add_file(MainSingleton.level_name + "/%s/" + file_name.get_file() % mode, "user://level_data/" + MainSingleton.level_name + "/%s/" + file_name.get_file() % mode)
